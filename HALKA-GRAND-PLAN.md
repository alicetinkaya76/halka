...
> **Versiyon:** v0.8.4 — F1.1 Pagefind static search canlıda. Header'da "Ara" butonu (⌘K kısayolu), modal dialog, Türkçe arama desteği (8530 kelime, 14 sayfa indeksli, ~1.1MB index). Halka design system'e uyumlu (emerald accent, Inter font, ink palette). Pagefind dist/'e otomatik ekleniyor (`build` scripti `astro build && pagefind --site dist`); GitHub Actions npm ci + npm run build zaten doğru zinciri çalıştırır, workflow değişikliği gerekmedi. v0.8.3 — F1.3 tarihyazımı 2D scatter (3 yatay eksen → tek SVG dağılım, 6 kitap aynı görselde, çöküşçüler kırmızı kenarlı, vertical jitter ile çakışma giderme) + F1.4 yer tutucusu (header 2-kolon, dashed-border placeholder). v0.8.2 — Faz 2 Kol A (feedback hızlı uyum): F1.6/F1.7/F1.8/F2.5 kapatıldı, şemaya `reading_level`+`accessibility_note`+`cover_image`+`cover_credit` eklendi, hibrit lisans (MIT+CC BY-SA 4.0), Pedersen disambiguation. v0.8.1 — Faz 0 KAPANDI: alicetinkaya76.github.io/halka canlıda. v0.8 — 1. dönem 6/6 = TAM (6/21 kitap toplam, Fara açılış kitabı + 60 atomik tartışma sorusu).
